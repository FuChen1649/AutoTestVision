...
class LogService:
    ...
    async def _count_monkey(self, db: AsyncSession) -> int:
    ...
    async def list_logs(
        ...
        *,
        ...
        if source in (None, "monkey") and not run_uuid:
            total += await self._count_monkey(db)
            query = (
    ...
    async def _total_for_source(self, db: AsyncSession, source: str, run_uuid: str | None) -> int:
        ...
        if source == "monkey":
            return await self._count_monkey(db)
