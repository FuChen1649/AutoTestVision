...
class MonkeyExplorer:
    async def explore(self, db: AsyncSession, session_uuid: str) -> AsyncGenerator[str, None]:
        ...
        await monkey_repository.add_log(db, session, "开始探索：返回 Home 并建立屏幕节点")
        await db.commit()

        yield self._sse(
            MonkeyStreamEvent(type="start", session=monkey_repository.session_to_response(session))
        )
        ...
        try:
            ...
            await monkey_repository.add_log(db, session, "已创建 Home 屏幕节点")
            await db.commit()
            yield await self._yield_state(db, session_uuid, "bootstrap")
            ...
            while step < session.max_steps:
                ...
                await monkey_repository.add_log(
                    db,
                    session,
                    f"步骤 {step}：导航真机至「{focus_screen.title}」",
                    step_index=step,
                    log_type="nav",
                )
                ...
                if not screen_actions:
                    ...
                    await monkey_repository.add_log(
                        db,
                        session,
                        f"步骤 {step}：首次进入「{focus_screen.title}」，识别任务相关元素",
                        step_index=step,
                        log_type="model",
                    )
                    await db.commit()
                    ...
                    await monkey_repository.add_log(
                        db,
                        session,
                        f"「{focus_screen.title}」识别到 {len(all_screen_actions)} 个元素（新增 {len(new_actions)}）",
                        step_index=step,
                        log_type="screen",
                    )
                    ...
                    await db.commit()
                    yield await self._yield_state(db, session_uuid, "step")
                    continue
                ...
                if not next_focus:
                    ...
                    await monkey_repository.add_log(
                        db, session, "所有屏幕已探索完毕", step_index=step
                    )
                    await db.commit()
                    yield await self._yield_state(db, session_uuid, "step")
                    break
    ...
    async def _yield_state(self, db: AsyncSession, session_uuid: str, event_type: str) -> str:
        session = await self._reload(db, session_uuid)
